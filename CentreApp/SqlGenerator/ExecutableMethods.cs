...
namespace SqlData.SqlGenerator
{
    public class ExecutableMethods 
    {
        ...
        public IEnumerable<T> GetDataProc<T>(string qry)
        {
            using (var db = new SqlConnection(conn))
            {
                var result = db.Query<T>(qry, commandType: CommandType.StoredProcedure);
                return result;
            }

        }
        // Get data from database with async
        public async Task<IEnumerable<T>> GetDataProcAsync<T>(string qry)
        {
            using (var db = new SqlConnection(conn))
            {
                var result = await db.QueryAsync<T>(qry, commandType: CommandType.StoredProcedure);
                return result;
            }
        ...
        public IEnumerable<T> GetDataProc<T>(string qry, object parameters)
        {
            using (var db = new SqlConnection(conn))
            {
                var result = db.Query<T>(qry, parameters, commandType: CommandType.StoredProcedure);
                return result;
            }

        }
        public async Task<IEnumerable<T>> GetDataProcAsync<T>(string qry, object parameters)
        {
            using (var db = new SqlConnection(conn))
            {
                var result = await db.QueryAsync<T>(qry, parameters, commandType: CommandType.StoredProcedure);
                return result;
            }
        ...
        public int ExecuteProc(string ProcName)
        {
            using (var db = new SqlConnection(conn))
            {
                var result = db.Execute(ProcName, commandType: CommandType.StoredProcedure);
                return result;
            }

        }
        // Execute the qry with parameters
        public int ExecuteProc(string ProcName, object parameters)
        {
            using (var db = new SqlConnection(conn))
            {
                var result = db.Execute(ProcName, parameters, commandType: CommandType.StoredProcedure);
                return result;
            }
        ...
        public async Task<int> ExecuteProcAsync(string ProcName)
        {
            using (var db = new SqlConnection(conn))
            {
                var result = await db.ExecuteAsync(ProcName, commandType: CommandType.StoredProcedure);
                return result;
            }
        ...
        public async Task<int> ExecuteProcAsync(string ProcName, object parameters)
        {
            using (var db = new SqlConnection(conn))
            {
                var result = await db.ExecuteAsync(ProcName, parameters, commandType: CommandType.StoredProcedure);
                return result;
            }
